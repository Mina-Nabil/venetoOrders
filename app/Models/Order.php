...
class Order extends Model
{
    ...
    private static function tableQuery()
    {
        return DB::table("orders")
            ...
            ->Leftjoin("drivers", "ORDR_DRVR_ID", "=", "drivers.id")
            ->join("payment_options", "ORDR_PYOP_ID", "=", "payment_options.id")
            ->select("orders.*", "order_status.STTS_NAME", "dash_users.DASH_USNM", "areas.AREA_NAME", "payment_options.PYOP_NAME", "ORSC_NAME", "DRVR_NAME")->selectRaw("SUM(ORIT_CUNT) as itemsCount")
    ...
    public static function getDetailedInventoryReport(DateTime $start, DateTime $end, $type = -1)
    {
        $query = DB::table("order_items", "t1")->join("orders", "ORIT_ORDR_ID", '=', 'orders.id')->join("finished", "ORIT_FNSH_ID", "=", "finished.id")
            ...
            ->selectRaw("SUM(ORIT_CUNT) as soldCount , AVG(ORIT_PRCE) as averagePrice , (AVG(ORIT_PRCE) * SUM(ORIT_CUNT)) as totalSold  ,  
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND oi2.ORIT_SIZE = 36 AND o2.ORDR_DLVR_DATE BETWEEN \"{$start->format('Y-m-01 00:00:00')}\" AND \"{$end->format('Y-m-t 23:59:59')}\" AND finished.id=f2.id GROUP BY f2.id) as total36 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND oi2.ORIT_SIZE = 38 AND o2.ORDR_DLVR_DATE BETWEEN \"{$start->format('Y-m-01 00:00:00')}\" AND \"{$end->format('Y-m-t 23:59:59')}\" AND finished.id=f2.id GROUP BY f2.id) as total38 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND oi2.ORIT_SIZE = 40 AND o2.ORDR_DLVR_DATE BETWEEN \"{$start->format('Y-m-01 00:00:00')}\" AND \"{$end->format('Y-m-t 23:59:59')}\" AND finished.id=f2.id GROUP BY f2.id) as total40 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND oi2.ORIT_SIZE = 42 AND o2.ORDR_DLVR_DATE BETWEEN \"{$start->format('Y-m-01 00:00:00')}\" AND \"{$end->format('Y-m-t 23:59:59')}\" AND finished.id=f2.id GROUP BY f2.id) as total42 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND oi2.ORIT_SIZE = 44 AND o2.ORDR_DLVR_DATE BETWEEN \"{$start->format('Y-m-01 00:00:00')}\" AND \"{$end->format('Y-m-t 23:59:59')}\" AND finished.id=f2.id GROUP BY f2.id) as total44 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND oi2.ORIT_SIZE = 46 AND o2.ORDR_DLVR_DATE BETWEEN \"{$start->format('Y-m-01 00:00:00')}\" AND \"{$end->format('Y-m-t 23:59:59')}\" AND finished.id=f2.id GROUP BY f2.id) as total46 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND oi2.ORIT_SIZE = 48 AND o2.ORDR_DLVR_DATE BETWEEN \"{$start->format('Y-m-01 00:00:00')}\" AND \"{$end->format('Y-m-t 23:59:59')}\" AND finished.id=f2.id GROUP BY f2.id) as total48 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND oi2.ORIT_SIZE = 50 AND o2.ORDR_DLVR_DATE BETWEEN \"{$start->format('Y-m-01 00:00:00')}\" AND \"{$end->format('Y-m-t 23:59:59')}\" AND finished.id=f2.id GROUP BY f2.id) as total50 ")
            ->groupBy("finished.id")
