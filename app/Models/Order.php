...
class Order extends Model
{
    ...
    public static function getDetailedInventoryReport(DateTime $start, DateTime $end, $type = -1)
    {
        $query = DB::table("order_items", "t1")->join("orders", "ORIT_ORDR_ID", '=', 'orders.id')->join("finished", "ORIT_FNSH_ID", "=", "finished.id")
            ...
            ->selectRaw("SUM(ORIT_CUNT) as soldCount , AVG(ORIT_PRCE) as averagePrice , (AVG(ORIT_PRCE) * SUM(ORIT_CUNT)) as totalSold  ,  
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND o2.ORDR_DLVR_DATE AND oi2.ORIT_SIZE = 36 AND BETWEEN {$start->format('Y-m-01 00:00:00')} AND {$end->format('Y-m-t 23:59:59')} GROUP BY f2.FNSH_BRND_ID, f2.FNSH_MODL_ID) as total36 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND o2.ORDR_DLVR_DATE AND oi2.ORIT_SIZE = 38 AND BETWEEN {$start->format('Y-m-01 00:00:00')} AND {$end->format('Y-m-t 23:59:59')} GROUP BY f2.FNSH_BRND_ID, f2.FNSH_MODL_ID) as total38 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND o2.ORDR_DLVR_DATE AND oi2.ORIT_SIZE = 40 AND BETWEEN {$start->format('Y-m-01 00:00:00')} AND {$end->format('Y-m-t 23:59:59')} GROUP BY f2.FNSH_BRND_ID, f2.FNSH_MODL_ID) as total40 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND o2.ORDR_DLVR_DATE AND oi2.ORIT_SIZE = 42 AND BETWEEN {$start->format('Y-m-01 00:00:00')} AND {$end->format('Y-m-t 23:59:59')} GROUP BY f2.FNSH_BRND_ID, f2.FNSH_MODL_ID) as total42 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND o2.ORDR_DLVR_DATE AND oi2.ORIT_SIZE = 44 AND BETWEEN {$start->format('Y-m-01 00:00:00')} AND {$end->format('Y-m-t 23:59:59')} GROUP BY f2.FNSH_BRND_ID, f2.FNSH_MODL_ID) as total44 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND o2.ORDR_DLVR_DATE AND oi2.ORIT_SIZE = 46 AND BETWEEN {$start->format('Y-m-01 00:00:00')} AND {$end->format('Y-m-t 23:59:59')} GROUP BY f2.FNSH_BRND_ID, f2.FNSH_MODL_ID) as total46 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND o2.ORDR_DLVR_DATE AND oi2.ORIT_SIZE = 48 AND BETWEEN {$start->format('Y-m-01 00:00:00')} AND {$end->format('Y-m-t 23:59:59')} GROUP BY f2.FNSH_BRND_ID, f2.FNSH_MODL_ID) as total48 ,
                        (SELECT SUM(ORIT_CUNT) FROM order_items as oi2, finished as f2, orders as o2 WHERE oi2.ORIT_FNSH_ID=f2.id AND oi2.ORIT_ORDR_ID = o2.id AND o2.ORDR_STTS_ID=4 AND o2.ORDR_DLVR_DATE AND oi2.ORIT_SIZE = 50 AND BETWEEN {$start->format('Y-m-01 00:00:00')} AND {$end->format('Y-m-t 23:59:59')} GROUP BY f2.FNSH_BRND_ID, f2.FNSH_MODL_ID) as total50 ")
            ->groupBy("m1.id", "b1.id")
