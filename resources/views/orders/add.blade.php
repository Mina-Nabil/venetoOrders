                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group mb-3">
                                <select name=source class="select2 form-control custom-select"
                                    style="width: 100%; height:36px;" required>
                                    <option value="" disabled selected>Pick From Order Sources</option>
                                    @foreach ($sources as $source)
                                        <option value="{{ $source->id }}"
                                            @if (old('source') == $source->id) selected @endif>
                                            {{ $source->ORSC_NAME }} @if ($source->client_account)
                                                - Account: {{ $source->client_account->CLNT_NAME }}
                                            @endif
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        {{-- @if ($isOnline) --}}
                            <div class="form-group">
                                ...
                                <div class="input-group mb-3">
                                    <input type="text" class="form-control" placeholder="Full Name" name=guestName
                                        value="{{ old('guestName') }}">
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group mb-3">
                                    <input type="text" class="form-control" placeholder="Client Mobile Number"
                                        name=guestMob value="{{ old('guestMob') }}">
                                </div>
                        ...
                        <div class="form-group">
                            <label>Area</label>
                            <div class="input-group mb-3">
                                <select name=area id=areaSel class="select2 form-control custom-select"
                                    style="width: 100%; height:36px;" required>
                                    <option value="" disabled selected>Pick From Areas</option>
                                    @foreach ($areas as $area)
                                        <option value="{{ $area->id }}"
                                            @if (old('area') == $area->id) selected @endif>{{ $area->AREA_NAME }} :
                                            {{ $area->AREA_ARBC_NAME }}</option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <div class=card>
                    <div class="card-body">
                        ...
                        @if ($isOnline)
                                ...
                                <div class="row col-lg-12">
                                    <div class="col-lg-3">
                                        <div class="input-group mb-2">
                                            <select name=item[] class="form-control select2  custom-select"
                                                style="width:100%" required>
                                                <option disabled hidden selected value="">Model</option>
                                                @foreach ($finished as $item)
                                                    <option value="{{ $item->id }}">
                                                        {{ $item->BRND_NAME }} - {{ $item->MODL_UNID }} - Price:
                                                        {{ $item->FNSH_PRCE }}EGP</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-lg-3">
                                        <div class="input-group mb-2">
                                            <select name=size[] class="form-control select2  custom-select"
                                                style="width:100%" required>
                                    ...
                                    <div class="col-lg-3">
                                        <div class="input-group mb-3">
                                            <input type="number" step=0.01 id=price class="form-control amount"
                                                placeholder="Item Price" min=0 name=price[] aria-describedby="basic-addon11"
                                                required>
                                    ...
                                    <div class="col-lg-3">
                                        <div class="input-group mb-3">
                                            <input type="number" step=1 id=count class="form-control amount"
                                                placeholder="Items Count" min=0 name=count[]
                                                aria-describedby="basic-addon11" required>
                                            <div class="input-group-append">
                                                <button class="btn btn-success" id="dynamicAddButton" type="button"
                                                    onclick="addOnlineToab();"><i class="fa fa-plus"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <button type="button" onclick="calculateOnlinePrice()"
                                class="btn btn-info mr-2">Calculate</button>
                        @else
                                    ...
                                    <div class="col-lg-2">
                                        <div class="input-group mb-3">
                                            <input type="number" step=0.01 class="form-control amount"
                                                placeholder="Price" name=price[] aria-label="Total Amount in Meters"
                                                aria-describedby="basic-addon11" required>
                                            <div class="input-group-append">
                                                <button class="btn btn-success" id="dynamicAddButton" type="button"
                                                    onclick="addOfflineToab();"><i class="fa fa-plus"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <button type="submit" class="btn btn-success mr-2">Submit</button>
                            <button type="button" onclick="calculateOfflineTotals()"
                                class="btn btn-info mr-2">Calculate</button>
                        @endif
                    </div>
                </div>
    ...
    <script>
        ...
        function addOnlineToab() {
            ...
            concatString +=
                " <div class='col-lg-3'>\
                                   <div class='input-group mb-3'>\
                                       <input type='number' step=1 class='form-control amount' placeholder='Items Count' min=0 id=count" +
                room + "\
                                           name=count[] \
                                           aria-describedby='basic-addon11' required>\
                                       <div class='input-group-append'>\
        ...
        function calculateOfflineTotals() {
            ...
            if (typeof prices.length === "undefined" && finished.selectedIndex != 0) {
                console.log("1 row")
                numberOfInv = Number(amount36.value) + Number(amount38.value) + Number(amount40.value) + Number(amount42
                    .value) + Number(amount44.value) + Number(amount46.value) + Number(amount48.value) + Number(amount50
                    .value);
                ...
                for (i; i < amount36.length; i++) {
                    if (finished[i].selectedIndex != 0) {

                        let row = Number(amount36[i].value) + Number(amount38[i].value) + Number(amount40[i].value) +
                            Number(amount42[i].value) + Number(amount44[i].value) + Number(amount46[i].value) + Number(
                                amount48[i].value) + Number(amount50[i].value);
                        numberOfInv += row;
                        ...
                        document.getElementById('itemsList').innerHTML += " <li class='list-group-item' >\
                                <div class='row'>\
                                    ...
                                    <div class='col-lg-4 p-l-10'> " + (prices[i].value * row).toFixed(1).replace(
                            /\d(?=(\d{3})+\.)/g, '$&,'); + "</div>\
                                </div>\
                                </li>";
                    }
        ...
        function addOfflineToab() {
            ...
            concatString += "<div class='col-lg-1'>\
                                        ...
                                        <div class='input-group mb-3'>\
                                            ...
                                            <div class='input-group-append'>\
                                                <button class='btn btn-danger' type='button' onclick='removeOfflineToab(" +
